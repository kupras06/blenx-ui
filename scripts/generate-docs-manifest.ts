...
// =====================
function generateRegistryJson(entry: {
  dir: string;
  meta: RegistryMetaJson;
}): void {
  ...
  const registryDependencies =
    meta.registryDependencies?.map(normalizeRegistryDependency) ?? [];
...
function generateSharedRegistry(): void {
  ...
  writeFileSync(
    join(REGISTRY_OUTPUT_DIR, "shared.json"),
...
function buildManifestInfo(metaPath: string): {
  ...
  const relativeCompDir = relative(
    BLOCK_COMPONENTS_DIR,
    compDir,
  );
...
`;
  ...
  console.log(
    `  ✔ demo registry written with ${allDemoImportPaths.length} demos`,
  );
  ...
  console.log(
    `  ✔ docs manifest written with ${Object.keys(manifest).length} components`,
  );
