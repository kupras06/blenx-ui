...
const staticRoutes = ["/", "/docs/installation"];

function loadComponentRoutes(): string[] {
  try {
    const manifest = JSON.parse(readFileSync("public/docs/components.json", "utf8"));
    return Object.keys(manifest).map((key) => `/components/${key}`);
  } catch {
    return [];
  }
}
