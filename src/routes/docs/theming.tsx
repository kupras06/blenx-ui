...
import { Separator, Surface, VStack, Text } from "@/components/ui";
import { DocsLayout } from "@/components/docs-layout";
import { theme, fontSize, spacing, lineHeight, fontWeight, fonts } from "@/lib/theme/theme.stylex";
...
const comparisonRows = [
  ["Configuration", "Requires Stylex bundler plugin", "None \u2014 just add a CSS file"],
  ["Type Safety", "Full type checking on token names", "None"],
  ["Light/Dark Mode", "Automatic via Stylex theme values", "Manual via media query in CSS"],
  ["Setup Effort", "Requires Stylex setup + TypeScript file", "Minimal"],
  ["Best For", "Full design system customization", "Quick overrides, non-Stylex projects"],
];
    ...
    <DocsLayout>
      <VStack gap="medium">
        ...
        <p {...stylex.props(styles.paragraph)}>
          ...
          The Stylex Contract Theme is the recommended approach for projects already using Stylex.
        </p>
        ...
        <div {...stylex.props(styles.section)}>
          <VStack gap="medium">
            <Text variant="h2">Approach 1: Stylex Contract Theme (Recommended)</Text>
            <p {...stylex.props(styles.paragraph)}>
              Create a typed theme using{" "}
              <code {...stylex.props(styles.inlineCode)}>stylex.createTheme</code> from the exported
              theme contract. This gives you full type safety and integrates directly with Stylex's
              theming system.
            </p>
            <p {...stylex.props(styles.paragraph)}>
              The generated theme is applied via a{" "}
              <code {...stylex.props(styles.inlineCode)}>className</code> on the root element. All
              child components using the contract tokens will automatically pick up your custom
              values.
            </p>

            <h3 {...stylex.props(styles.heading3)}>1. Create a theme file</h3>
            <Surface variant="sunken" padding="medium">
              <code {...stylex.props(styles.code)}>{`// styles/my-theme.stylex.ts
import * as stylex from "@stylexjs/stylex";
...
export const myTheme = stylex.createTheme(theme, {
  ...
  focusRing: "#6C63FF",
            ...
            <Surface variant="sunken" padding="medium">
              <code {...stylex.props(styles.code)}>{`// React — wrap your app root
import * as stylex from "@stylexjs/stylex";
    ...
    <div {...stylex.props(myTheme)}>
      <YourApp />
            ...
            <div {...stylex.props(styles.codeBlockMargin)}>
              <Surface variant="sunken" padding="medium">
                <code {...stylex.props(styles.code)}>{`// Next.js — apply to the body or a layout wrapper
// app/layout.tsx
import { myTheme } from "@/styles/my-theme.stylex";
    ...
    <html lang="en">
      <body className={myTheme}>
        {children}
            ...
            <Surface variant="sunken" padding="medium">
              <code {...stylex.props(styles.code)}>{`// Import the contract for createTheme
import { theme } from "@blenx-dev/theme/theme.stylex";
...
import { theme, borderRadius, spacing, fontSize } from "@/lib/theme/tokens.stylex";`}</code>
        ...
        <div {...stylex.props(styles.section)}>
          <VStack gap="medium">
            ...
            <Surface variant="sunken" padding="medium">
              <code {...stylex.props(styles.code)}>{`/* my-theme.css — import or link after the component styles */
:root {
  --primary: #1a1a2e;
  --secondary: #e94560;
  --background: #f8f9fa;
  ...
  :root {
    ...
    --focusRing: #e94560;
            ...
            <p {...stylex.props(styles.compactP)}>
              All semantic color tokens from the theme contract. Override any subset &mdash; the rest
                    ...
                    <tr key={row.join("")}>
                      ...
                      {row.length < 4
                        ? Array.from({ length: 4 - row.length }).map((_, i) => (
                          <td key={`empty-${i}`} {...stylex.props(styles.td)} />
                        ))
                        : null}
                    </tr>
                  ))}
