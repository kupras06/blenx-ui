...
Always provide the `name` prop so the initials fallback is meaningful. A generic user icon communicates nothing about the person. Set `alt` text to the person's name for screen readers; decorative alt text like "User avatar" adds no value. Use the smallest size that remains legible for initials — two characters at 16px is illegible, so choose sm (24px) or larger. For status indicators, choose colors that map to universally understood meanings: green for online, amber for away, red for busy, gray for offline.

## Common Mistakes

Relying on Avatar as the sole identifier in lists. Users with identical initials become indistinguishable. Always pair avatars with text labels in data-heavy contexts. Another mistake is omitting the `name` prop, which degrades the fallback to a generic icon on image load failure. Providing images with inconsistent aspect ratios causes distortion; the component applies `object-fit: cover` but source images should still be roughly square for best results.
