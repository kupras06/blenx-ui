...
Always communicate what is happening through visible label text, either as an adjacent element or via `aria-label`. Use determinate mode whenever the progress value is known — it provides genuine information. Indeterminate mode should include a status message indicating that work is ongoing. Avoid showing multiple progress bars simultaneously for the same operation, as this creates confusion about which bar represents the overall status.

## Common Mistakes

Animating the progress bar from 0 to 100 without real progress data is deceptive and reduces trust. Using an indeterminate progress bar when the operation duration is known wastes the opportunity to inform the user. Failing to announce progress changes to screen readers via live regions leaves assistive technology users uninformed about operation status.
